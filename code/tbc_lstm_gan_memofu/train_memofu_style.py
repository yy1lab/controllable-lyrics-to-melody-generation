...
def main():
    ...
    num_pitch_variance_tokens = create_k_bins_encoder(p_var, P_VAR_PATH, k=70)
    ...
    adv_train_driver = AdversarialDriverWithStyle(g_model,
                                                  d_model,
                                                  adv_train_g_opt,
                                                  adv_train_d_opt,
                                                  TEMP_MAX,
    ...
    pre_train_ckpt_manager = tf.train.CheckpointManager(
        pre_train_ckpt, MEMOFU_STYLE_PRE_TRAIN_CKPT_PATH, max_to_keep=PRE_TRAIN_EPOCHS)
    ...
    adv_train_ckpt_manager = tf.train.CheckpointManager(
        adv_train_ckpt, MEMOFU_STYLE_ADV_TRAIN_CKPT_PATH, max_to_keep=ADV_TRAIN_EPOCHS)

    ## Setup Logging

    os.makedirs(MEMOFU_STYLE_LOG_DIR, exist_ok=True)
    previous_runs = os.listdir(MEMOFU_STYLE_LOG_DIR)
    ...
    memofu_summary_writer = tf.summary.create_file_writer(os.path.join(MEMOFU_STYLE_LOG_DIR, logdir))
    ...
    for epoch in range(ADV_TRAIN_EPOCHS):
        ...
        if epoch % EVAL_INTERVAL == 0:
            ...
            # generated val. song attr.
            valid_g_loss, valid_d_loss, valid_g_out = adv_train_driver.test_step((x_valid_and_style, y_valid))
