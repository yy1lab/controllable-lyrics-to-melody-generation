...
def main():
    ...
    print("Shape of rest data:", y_r.shape)

    p_rng, p_avg, p_var = calculate_style_features(y_p, title_str='Pitch', plot=1)
    ...
    num_pitch_variance_tokens = create_k_bins_encoder(p_var, P_VAR_PATH, k=30)
    ...
    # train_style_p = np.concatenate([train_p_rav[0], train_p_rav[1], train_p_rav[2]], axis=-1)
    # train_style_d = np.concatenate([train_d_rav[0], train_d_rav[1], train_d_rav[2]], axis=-1)
    # train_style_r = np.concatenate([train_r_rav[0], train_r_rav[1], train_r_rav[2]], axis=-1)
    train_style = (train_p_rav, train_d_rav, train_r_rav)
    valid_style = (valid_p_rav, valid_d_rav, valid_r_rav)
    test_style = (test_p_rav, test_d_rav, test_r_rav)

    x_train_and_style = (x_train, train_style)
    x_valid_and_style = (x_valid, valid_style)

    train_dataset = tf.data.Dataset.from_tensor_slices((x_train_and_style, y_train))
    ...
    g_model = GeneratorWithMemoryFusionAndStyle(
    ...
    d_model = DiscriminatorWithStyle(
    ...
    pre_train_driver = PreTrainDriverWithStyle(g_model,
                                               pre_train_g_opt,
                                               NUM_TOKENS,
                                               MAX_GRAD_NORM)

    # Initialise adversarial driver
    adv_train_driver = AdversarialDriverWithStyleAndSeqLoss(g_model,
                                                            d_model,
                                                            adv_train_g_opt,
                                                            adv_train_d_opt,
                                                            seq_train_opt,
    ...
    pre_train_ckpt_manager = tf.train.CheckpointManager(
        pre_train_ckpt, MEMOFU_STYLE_SEQ_PRE_TRAIN_CKPT_PATH, max_to_keep=PRE_TRAIN_EPOCHS)
    ...
    adv_train_ckpt_manager = tf.train.CheckpointManager(
        adv_train_ckpt, MEMOFU_STYLE_SEQ_ADV_TRAIN_CKPT_PATH, max_to_keep=ADV_TRAIN_EPOCHS)

    ## Setup Logging

    os.makedirs(MEMOFU_STYLE_SEQ_LOG_DIR, exist_ok=True)
    previous_runs = os.listdir(MEMOFU_STYLE_SEQ_LOG_DIR)
    ...
    memofu_summary_writer = tf.summary.create_file_writer(os.path.join(MEMOFU_STYLE_SEQ_LOG_DIR, logdir))
    ...
    for epoch in range(PRE_TRAIN_EPOCHS):
        ...
        if epoch % EVAL_INTERVAL == 0:
            ...
            # perform validation
            valid_p_loss, valid_d_loss, valid_r_loss = pre_train_driver.test_step((x_valid_and_style, y_valid))

            # generated val. song attr.
            valid_g_out = adv_train_driver.generate(x_valid, valid_style)
            ...
            test_g_out = adv_train_driver.generate(x_test, test_style)
    ...
    for epoch in range(ADV_TRAIN_EPOCHS):
        ...
        total_train_d_loss = 0
        total_train_seq_loss_p = 0
        total_train_seq_loss_d = 0
        total_train_seq_loss_r = 0

        for step, inp in enumerate(train_dataset.take(STEPS_PER_EPOCH_TRAIN)):
            # update temperature
            adv_train_driver.update_temp(epoch, step)

            batch_g_loss, batch_d_loss, batch_seq_loss, batch_g_out = adv_train_driver.train_step(inp)
            total_train_g_loss += batch_g_loss
            total_train_d_loss += batch_d_loss
            total_train_seq_loss_p += batch_seq_loss[0]
            total_train_seq_loss_d += batch_seq_loss[1]
            total_train_seq_loss_r += batch_seq_loss[2]
        ...
        train_d_loss = total_train_d_loss/STEPS_PER_EPOCH_TRAIN
        train_seq_loss_p = total_train_seq_loss_p/STEPS_PER_EPOCH_TRAIN
        train_seq_loss_d = total_train_seq_loss_d/STEPS_PER_EPOCH_TRAIN
        train_seq_loss_r = total_train_seq_loss_r/STEPS_PER_EPOCH_TRAIN
        ...
        if epoch % EVAL_INTERVAL == 0:
            ...
            with memofu_summary_writer.as_default():
                ...
                tf.summary.scalar('seq_train/seq_loss_p', train_seq_loss_p, step=epoch)
                tf.summary.scalar('seq_train/seq_loss_d', train_seq_loss_d, step=epoch)
                tf.summary.scalar('seq_train/seq_loss_r', train_seq_loss_r, step=epoch)

            # generated val. song attr.
            valid_g_loss, valid_d_loss, valid_seq_loss, valid_g_out = adv_train_driver.test_step((x_valid_and_style, y_valid))
            valid_seq_loss_p, valid_seq_loss_d, valid_seq_loss_r = valid_seq_loss
            ...
            with memofu_summary_writer.as_default():
                ...
                tf.summary.scalar('seq_valid/seq_loss_p', valid_seq_loss_p, step=epoch)
                tf.summary.scalar('seq_valid/seq_loss_d', valid_seq_loss_d, step=epoch)
                tf.summary.scalar('seq_valid/seq_loss_r', valid_seq_loss_r, step=epoch)
            ...
            with memofu_summary_writer.as_default():
                for n_gram in N_GRAMS:
                    tf.summary.scalar(f'SB_test/selfBLEU_{n_gram}', test_self_bleu[n_gram], step=epoch+PRE_TRAIN_EPOCHS)
            ...
            print('Epoch {} Train seq loss p: {:.4f}, seq loss d: {:.4f}, seq loss r: {:.4f}'.format(
                epoch+1, train_seq_loss_p, train_seq_loss_d, train_seq_loss_r))

            print('Epoch {} Valid seq loss p: {:.4f}, seq loss d: {:.4f}, seq loss r: {:.4f}'.format(
                epoch+1, valid_seq_loss_p, valid_seq_loss_d, valid_seq_loss_r))
