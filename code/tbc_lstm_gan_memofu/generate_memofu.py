...
def main():
    ...
    style_seq_adv_train_ckpt_manager = tf.train.CheckpointManager(
        style_seq_adv_train_ckpt, CKPT_PATH, max_to_keep=ADV_TRAIN_EPOCHS)
    ...
    if MIDI_NAME:
        gen_midi.write(f'../../subjective_evaluation/quality/{MIDI_NAME + style_str}.mid')
        print(f"Melody can be found at ../../subjective_evaluation/quality/{MIDI_NAME}.mid")
    else:
        timestamp = time.time()
        gen_midi.write(f'../../subjective_evaluation/quality/{str(timestamp) + style_str}.mid')
...
if __name__ == '__main__':
    ...
    parser.add_argument("--CKPT_PATH",   help="path to the model checkpoints.", type=str,
                        default='../../checkpoints/adv_train_tbc_lstm_gan_memofu_style_seq')
    parser.add_argument("--MIDI_NAME", help="name of the generated melody", type=str, default=None)
