...
def main():
    ...
    adv_train_driver = AdversarialDriverWithSeqLoss(g_model,
                                                    d_model,
                                                    adv_train_g_opt,
                                                    adv_train_d_opt,
                                                    seq_train_opt,
    ...
    pre_train_ckpt_manager = tf.train.CheckpointManager(
        pre_train_ckpt, MEMOFU_SEQ_PRE_TRAIN_CKPT_PATH, max_to_keep=PRE_TRAIN_EPOCHS)
    ...
    adv_train_ckpt_manager = tf.train.CheckpointManager(
        adv_train_ckpt, MEMOFU_SEQ_ADV_TRAIN_CKPT_PATH, max_to_keep=ADV_TRAIN_EPOCHS)

    ## Setup Logging

    os.makedirs(MEMOFU_SEQ_LOG_DIR, exist_ok=True)
    previous_runs = os.listdir(MEMOFU_SEQ_LOG_DIR)
    ...
    memofu_summary_writer = tf.summary.create_file_writer(os.path.join(MEMOFU_SEQ_LOG_DIR, logdir))
    ...
    for epoch in range(ADV_TRAIN_EPOCHS):
        ...
        train_seq_loss = train_seq_loss_p, train_seq_loss_d, train_seq_loss_r
        ...
        if epoch % EVAL_INTERVAL == 0:
            ...
            # generated val. song attr.
            valid_g_loss, valid_d_loss, valid_seq_loss, valid_g_out = adv_train_driver.test_step((x_valid, y_valid))
