...
@File: subjective_controllability.py
@Author: Zhe Zhang
@Email: zhe@nii.ac.jp
@Date: 2022/10/25
"""
...
def main():
    ...
    if MIDI_NAME:
        gen_midi.write(f'../../subjective_evaluation/controllability/{MODEL + MIDI_NAME + style_str}.mid')
        print(f"Melody can be found at ../../subjective_evaluation/controllability/{MIDI_NAME}.mid")
    else:
        timestamp = time.time()
        gen_midi.write(f'../../subjective_evaluation/controllability/{MODEL + str(timestamp) + style_str}.mid')
        print(f"Melody can be found at ../../subjective_evaluation/controllability/{timestamp}.mid")
...
if __name__ == '__main__':
    ...
    # STYLE_PITCH = ['0.3', '0.6', '0.2']
    # STYLE_DURATION = ['0.2', '0.2', '0.1']
    # STYLE_REST = ['0.1', '0.1', '0.1']
    STYLE_PITCH = [0.33, 0.66, 0.33]
    STYLE_DURATION = [0.33, 0.22, 0.11]
    STYLE_REST = [0.1, 0.1, 0.1]

    # STYLE_PITCH_LIST_1 = [['0.3', '0.2', '0.2'], ['0.3', '0.5', '0.2'], ['0.3', '0.8', '0.2']]
    # STYLE_PITCH_LIST_2 = [['0.2', '0.6', '0.2'], ['0.5', '0.6', '0.5'], ['0.8', '0.6', '0.8']]
    # STYLE_DURATION_LIST_1 = [['0.2', '0.8', '0.1'], ['0.2', '0.5', '0.1'], ['0.2', '0.2', '0.1']]
    # STYLE_DURATION_LIST_2 = [['0.2', '0.3', '0.2'], ['0.5', '0.3', '0.5'], ['0.8', '0.3', '0.8']]
    ...
    for i, STYLE_PITCH_CTRL in enumerate(STYLE_PITCH_LIST_1):

        settings['MIDI_NAME'] = NAME + '_ex1_' + str(i+1)

        settings['STYLE_PITCH'] = STYLE_PITCH_CTRL[0], STYLE_PITCH_CTRL[1], STYLE_PITCH_CTRL[2]
        settings['STYLE_DURATION'] = STYLE_DURATION[0], STYLE_DURATION[1], STYLE_DURATION[2]
        settings['STYLE_REST'] = STYLE_REST[0], STYLE_REST[1], STYLE_REST[2]
    ...
    for i, STYLE_PITCH_CTRL in enumerate(STYLE_PITCH_LIST_2):

        settings['MIDI_NAME'] = NAME + '_ex2_' + str(i+1)
    ...
    for i, STYLE_DURATION_CTRL in enumerate(STYLE_DURATION_LIST_1):

        settings['MIDI_NAME'] = NAME + '_ex3_' + str(i+1)

        settings['STYLE_PITCH'] = STYLE_PITCH[0], STYLE_PITCH[1], STYLE_PITCH[2]
        settings['STYLE_DURATION'] = STYLE_DURATION_CTRL[0], STYLE_DURATION_CTRL[1], STYLE_DURATION_CTRL[2]
        settings['STYLE_REST'] = STYLE_REST[0], STYLE_REST[1], STYLE_REST[2]
    ...
    for i, STYLE_DURATION_CTRL in enumerate(STYLE_DURATION_LIST_2):

        settings['MIDI_NAME'] = NAME + '_ex4_' + str(i+1)
